  ...
  switch (command) {
    ...
    case 'create-admin':
      ...
      if (!newUsername || !newPassword || !email) {
        console.error(
          'Uso: npm run user:create-admin <username> <password> <email>',
        );
        process.exit(1);
      }
      ...
      try {
        ...
        if (existingUser) {
          console.error(
            `❌ Error: El usuario '${newUsername}' o el email '${email}' ya existe`,
          );
          process.exit(1);
        }
    ...
    default:
      ...
      console.log(
        '  create-admin <username> <password> <email> - Crear nuevo administrador',
      );
  }
